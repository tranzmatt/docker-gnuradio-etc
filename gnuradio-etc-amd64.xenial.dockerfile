...
FROM ubuntu:bionic AS gnuradio
...
RUN apt-get update && apt-get install -y python-mako python-numpy python-requests python-cheetah
...
RUN pybombs -vv install mako numpy requests
